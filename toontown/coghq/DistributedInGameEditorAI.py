...
class DistributedInGameEditorAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def requestCurrentLevelSpec(self):
        ...
        specStr = repr(spec)
        largeBlob = DistributedLargeBlobSenderAI.DistributedLargeBlobSenderAI(self.air, self.zoneId, self.editorAvId, specStr, useDisk=config.ConfigVariableBool('spec-by-disk', 1).getValue())

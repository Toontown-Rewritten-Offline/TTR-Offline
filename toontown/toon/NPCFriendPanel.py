...
class NPCFriendCard(DirectFrame):
    ...
    def __init__(self, parent = aspect2dp, rotateCard = False, **kw):
        ...
        logo = loader.loadModel('phase_3/models/gui/toontown-logo')
        self.front = DirectFrame(parent=self, relief=None, image=cardModel.find('**/card_front'))
        self.front.hide()
        self.back = DirectFrame(parent=self, relief=None, image=cardModel.find('**/card_back'), geom=logo.find('**/eyes'))
        callButtonPosZ = -0.9
        textWordWrap = 16.0
        textScale = 0.35
        textPosZ = 1.15
        ...
        if rotateCard:
            ...
            self.back.component('geom0').configure(pos=(-0.07, 0, 0.25), scale=(0.65, 0.65, 0.65))
            callButtonPosZ = -2.4
            textWordWrap = 10.0
            textScale = 0.35
            textPosZ = 2.35
            ...
            rarityPosZ = -2.2
            self.NPCHeadDim = 2.3
            self.NPCHeadPosZ = 0.8
            self.sosCountInfoPosZ = -1.8
            self.sosCountInfoScale = 0.4
            self.sosCountInfo2PosZ = -1.7
        ...
        self.sosCallButton = DirectButton(parent=self.front, relief=None, text=TTLocalizer.NPCCallButtonLabel, text_fg=self.normalTextColor, text_scale=0.28, text_align=TextNode.ACenter, image=(upButton,
         downButton,
         rolloverButton,
         upButton), image_color=(1.0, 0.2, 0.2, 1), image0_color=Vec4(1.0, 0.4, 0.4, 1), image3_color=Vec4(1.0, 0.4, 0.4, 0.4), image_scale=(4.4, 1, 3.6), image_pos=Vec3(0, 0, 0.08), pos=(-1.4, 0, callButtonPosZ), scale=1.6, command=self.__chooseNPCFriend)
        ...
        for i in range(self.maxRarity):
            label = DirectLabel(parent=self.front, relief=None, image=star, image_scale=rarityScale, image_color=Vec4(0.502, 0.251, 0.251, 1.0), pos=(1.5 - i * 0.40, 0, rarityPosZ))
            label.hide()

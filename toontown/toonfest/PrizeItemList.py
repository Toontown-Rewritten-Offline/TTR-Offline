...
class PrizeItemList:
    ...
    def __init__(self, source = None, store = 0):
        ...
        if isinstance(source, bytes):
            ...
            self.__list = source[:]
        elif isinstance(source, PrizeItemList):
    ...
    def extractDeliveryItems(self, cutoffTime):
        ...
        return (PrizeItemList(beforeTime, store=self.store), PrizeItemList(afterTime, store=self.store))
    ...
    def __makeBlob(self, store):
        dg = PyDatagram()
        if self.__list:
            dg.addUint8(PrizeItem.PrizeItemVersion)
            for item in self.__list:
                PrizeItem.encodePrizeItem(dg, item, store)
    ...
    def __makeList(self, store):
        ...
        if self.__blob:
            ...
            while di.getRemainingSize() > 0:
                item = PrizeItem.decodePrizeItem(di, versionNumber, store)
                mList.append(item)
    ...
    def __getslice__(self, i, j):
        if self.__list == None:
            self.__decodeList()
        return PrizeItemList(self.__list[i:j], store=self.store)

    def __setslice__(self, i, j, s):
        if self.__list == None:
            self.__decodeList()
        if isinstance(s, PrizeItemList):
    ...
    def __add__(self, other):
        copy = PrizeItemList(self, store=self.store)
        copy += other
        return copy
    ...
    def output(self, store = -1):
        ...
        return 'PrizeItemList([%s])' % inner[2:]

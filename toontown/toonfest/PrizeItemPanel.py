...
PRIZE_PANEL_WORDWRAP = 10
PRIZE_PANEL_CHAT_WORDWRAP = 9
PRIZE_PANEL_ACCESSORY_WORDWRAP = 11

class PrizeItemPanel(DirectFrame):
    notify = DirectNotifyGlobal.directNotify.newCategory('PrizeItemPanel')

    def __init__(self, parent = aspect2d, parentPrizeScreen = None, **kw):
        optiondefs = (('item', None, DGG.INITOPT), ('type', PrizeItem.PrizeTypeUnspecified, DGG.INITOPT), ('relief', None, None))
        self.parentPrizeScreen = parentPrizeScreen
        self.defineoptions(kw, optiondefs)
        DirectFrame.__init__(self, parent)
        self.loaded = 0
        self.initialiseoptions(PrizeItemPanel)
        return
    ...
    def load(self):
        ...
        if self['item'].needsCustomize() and (typeCode == PrizeItemTypes.WALLPAPER_ITEM or typeCode == PrizeItemTypes.FLOORING_ITEM or typeCode == PrizeItemTypes.MOULDING_ITEM or typeCode == PrizeItemTypes.FURNITURE_ITEM or typeCode == PrizeItemTypes.WAINSCOTING_ITEM or typeCode == PrizeItemTypes.TOON_STATUE_ITEM):
            #if typeCode == PrizeItemTypes.WALLPAPER_ITEM:
            #    self.items = getAllWallpapers(self['item'].patternIndex)
            #elif typeCode == PrizeItemTypes.FLOORING_ITEM:
            #    self.items = getAllFloorings(self['item'].patternIndex)
            #elif typeCode == PrizeItemTypes.MOULDING_ITEM:
            #    self.items = getAllMouldings(self['item'].patternIndex)
            #elif typeCode == PrizeItemTypes.FURNITURE_ITEM:
            #    self.items = getAllFurnitures(self['item'].furnitureType)
            #elif typeCode == PrizeItemTypes.TOON_STATUE_ITEM:
            #    self.items = self['item'].getAllToonStatues()
            #elif typeCode == PrizeItemTypes.WAINSCOTING_ITEM:
        ...
        self.typeLabel = DirectLabel(parent=self, relief=None, pos=(0, 0, 0.24), scale=TTLocalizer.CIPtypeLabel, text=self['item'].getTypeName(), text_fg=(0.95, 0.95, 0, 1), text_shadow=(0, 0, 0, 1), text_font=ToontownGlobals.getInterfaceFont(), text_wordwrap=PRIZE_PANEL_WORDWRAP)
        self.auxText = DirectLabel(parent=self, relief=None, scale=0.05, pos=(-0.2, 0, 0.16))
        self.auxText.setHpr(0, 0, -30)
        self.nameLabel = DirectLabel(parent=self, relief=None, text=self['item'].getDisplayName(), text_fg=(0, 0, 0, 1), text_font=ToontownGlobals.getInterfaceFont(), text_scale=TTLocalizer.CIPnameLabel, text_wordwrap=PRIZE_PANEL_WORDWRAP + TTLocalizer.CIPwordwrapOffset)
        if self['item'].getTypeCode() == PrizeItemTypes.THROWABLE_ITEM:
            self.nameLabel['text_wordwrap'] = PRIZE_PANEL_CHAT_WORDWRAP
            ...
            if numRows == 1:
                ...
                namePos = (0, 0, 0)
            nameScale = 0.063
        #elif self['item'].getTypeCode() == PrizeItemTypes.ACCESSORY_ITEM:
        #    self.nameLabel['text_wordwrap'] = PRIZE_PANEL_ACCESSORY_WORDWRAP
        #    namePos = (0, 0, -.22)
        #    nameScale = 0.06
        #else:
        namePos = (0, 0, -.22)
        ...
        priceStr = str(numericBeanPrice) + ' ' + TTLocalizer.PrizeCurrency
        priceScale = 0.07
        ...
        buyText = TTLocalizer.PrizeBuyText
    ...
    def showCurrentVariant(self):
        ...
        if self.ival:
            self.ival.loop()
        if self['item'].getTypeCode() == PrizeItemTypes.TOON_STATUE_ITEM:
    ...
    def updateBuyButton(self):
        ...
        #else:
        #    auxText = ''
        #isNameTag = typeCode == PrizeItemTypes.NAMETAG_ITEM
        ...
        if self['item'].reachedPurchaseLimit(base.localAvatar):
            ...
            if max <= 1:
                auxText = TTLocalizer.PrizePurchasedText
    ...
    def __handlePurchaseRequest(self):
        if self['item'].replacesExisting() and self['item'].hasExisting():
            ...
            message = TTLocalizer.PrizeVerifyPurchase % {'item': self['item'].getName(),
    ...
    def __handleVerifyPurchase(self):
        if config.GetBool('want-qa-regression', 0):
            self.notify.info('QA-REGRESSION: PRIZE: Order item')
        ...
        if status == 'ok':
            ...
            messenger.send('PrizeItemPurchaseRequest', [item])
            self.buyButton['state'] = DGG.DISABLED
        return
    ...
    def __handleGiftRequest(self):
        ...
        #     'price': self['item'].getPrice(self['type'])}
        #else:
        #    friendIndex = self.parentPrizeScreen.friendGiftIndex
        #    friendText = 'Error'
        #    numFriends = len(base.localAvatar.friendsList) + len(base.cr.avList) - 1
        #    if numFriends > 0:
        #        friendText = self.parentPrizeScreen.receiverName
    ...
    def __handleVerifyGift(self):
        if config.GetBool('want-qa-regression', 0):
            self.notify.info('QA-REGRESSION: PRIZE: Gift item')
        status = self.verify.doneStatus
        ...
        if status == 'ok':
            ...
            item = self.items[self.itemIndex]
            messenger.send('PrizeItemGiftPurchaseRequest', [item])
        return

    def updateButtons(self, giftActivate = 0):
        #if self.parentPrizeScreen.gifting == -1:
    ...
    #            self.auxText['text'] = auxText
    #            return
    #        elif self.parentPrizeScreen.gotAvatar == 1:
    #            avatar = self.parentPrizeScreen.giftAvatar

...
from . import PrizeItem
from . import PrizeInvalidItem
from . import PrizeItemList
from . import PrizeThrowableItem
from toontown.toonbase import TTLocalizer
from . import PrizeItemPanel
...
NUM_PRIZE_ROWS = 3
NUM_PRIZE_COLS = 2
PrizePanelCenters = [[Point3(-0.95, 0, 0.91), Point3(-0.275, 0, 0.91)], [Point3(-0.95, 0, 0.275), Point3(-0.275, 0, 0.275)], [Point3(-0.95, 0, -0.4), Point3(-0.275, 0, -0.4)]]
#PrizePanelColors = {PrizeItemTypes.FURNITURE_ITEM: Vec4(0.733, 0.78, 0.933, 1.0),
# PrizeItemTypes.CHAT_ITEM: Vec4(0.922, 0.922, 0.753, 1.0),
# PrizeItemTypes.CLOTHING_ITEM: Vec4(0.918, 0.69, 0.69, 1.0),
# PrizeItemTypes.EMOTE_ITEM: Vec4(0.922, 0.922, 0.753, 1.0),
# PrizeItemTypes.WALLPAPER_ITEM: Vec4(0.749, 0.984, 0.608, 1.0),
# PrizeItemTypes.WINDOW_ITEM: Vec4(0.827, 0.91, 0.659, 1.0)}
...
class PrizeScreen(DirectFrame):
    ...
    def __init__(self, parent = aspect2d, prizes=False, **kw):
        ...
        self.initialiseoptions(PrizeScreen)
    ...
    def show(self):
        self.accept('PrizeItemPurchaseRequest', self.__handlePurchaseRequest)
        self.accept('PrizeItemGiftPurchaseRequest', self.__handleGiftPurchaseRequest)
    ...
    def hide(self):
        if not self.prizes:
            self.ignore('PrizeItemPurchaseRequest')
            self.ignore('PrizeItemGiftPurchaseRequest')
    ...
    def showNewItems(self, index = None):
        if config.GetBool('want-qa-regression', 0):
            self.notify.info('QA-REGRESSION: PRIZE: New item')
    ...
    def showBackorderItems(self, index = None):
        if config.GetBool('want-qa-regression', 0):
            self.notify.info('QA-REGRESSION: PRIZE: Backorder item')
        taskMgr.remove('clarabelleHelpText1')
        messenger.send('wakeup')
    ...
    def showLoyaltyItems(self, index = None):
        if config.GetBool('want-qa-regression', 0):
            self.notify.info('QA-REGRESSION: PRIZE: Special item')
        taskMgr.remove('clarabelleHelpText1')
        messenger.send('wakeup')
    ...
    def showEmblemItems(self, index = None):
        if config.GetBool('want-qa-regression', 0):
            self.notify.info('QA-REGRESSION: PRIZE: Emblem item')
        taskMgr.remove('clarabelleHelpText1')
        messenger.send('wakeup')
        self.viewing = 'Emblem'
    ...
    def showPageItems(self):
        ...
        if self.pageIndex < 0:
            ...
            #for i in range(NUM_PRIZE_ROWS):
            #    for j in range(NUM_PRIZE_COLS):
            #        if not self.prizes:
            #            if pIndex < len(self.visiblePanels):
            #                type = self.visiblePanels[pIndex]['item'].getTypeCode()
            #                self.squares[i][j].setColor(list(PrizePanelColors.values())[randGen.randint(0, len(PrizePanelColors) - 1)])
            ...
            #            else:
            #                self.squares[i][j].setColor(PrizePanelColors[PrizeItemTypes.CHAT_ITEM])
    ...
    def adjustForSound(self):
        ...
        for visIndex in range(len(self.visiblePanels)):
            ...
            item = panel['item']
            prizeType = item.getTypeCode()
            #if prizeType == PrizeItemTypes.EMOTE_ITEM:
            #    numEmoteItems += 1
    ...
    def packPages(self, panelList, pageList, prefix):
        ...
        for item in panelList:
            if i == 0 and j == 0:
                ...
                page.reparentTo(self.prizePanel)
                page.setScale(0.75)
                page.setPos(0.25, 0, -0.2)
                print('page: %s' % page)
                self.pageList.append(page)
            item.reparentTo(page)
            item.setPos(PrizePanelCenters[i][j])
    ...
    def load(self, guiItems, guiButton, guiBack):
        ...
        if not self.prizes:
            ...
            self.base = DirectLabel(self, relief=None, image=None)
        ...
        self.prizePanelMain = self.tokenGUI.find('**/prizePanelMain')
        self.prizePanel = DirectFrame(relief=None, image=self.prizePanelMain, pos=(-0.5, 0, 0))
        ...
        self.cancelButton = DirectButton(parent=self.prizePanel, image=(self.cancelIcon,
         self.cancelPressed,
         self.cancelRollover), relief=None, command=self.hangUp)
        ...
        def priceSort(item): # PY3
            return item.getPrice(PrizeItem.PrizeTypePies)


        #itemList = PrizeItemList.PrizeItemList(weeklyCatalog).getAllItems()
        itemList = base.localAvatar.weeklyCatalog.getAllItems()
        ...
        for item in itemList:
            #if isinstance(item, PrizeInvalidItem.PrizeInvalidItem):
            #    self.notify.warning('skipping prize invalid item %s' % item)
            #    continue
            #if isMaxClosetOfferred and item in allClosetItems and item.furnitureType not in CatalogFurnitureItem.MaxClosetIds:
            #    continue
            self.panelList.append(PrizeItemPanel.PrizeItemPanel(parent=hidden, item=item, type=PrizeItem.PrizeTypePies, parentPrizeScreen=self))

        #itemList = base.localAvatar.backCatalog
        #itemList.sort(lambda a, b: priceSort(a, b, PrizeItem.prizeTypeBackorder))
        #itemList.reverse()
        ...
        #        self.loyaltyPanelList.append(PrizeItemPanel.PrizeItemPanel(parent=hidden, item=item, type=PrizeItem.prizeTypeLoyalty, parentPrizeScreen=self))
        #    elif item.getEmblemPrices():
        #        self.emblemPanelList.append(PrizeItemPanel.PrizeItemPanel(parent=hidden, item=item, type=PrizeItem.prizeTypeBackOrder, parentPrizeScreen=self))
        #    else:
        #        self.backPanelList.append(PrizeItemPanel.PrizeItemPanel(parent=hidden, item=item, type=PrizeItem.prizeTypeBackorder, parentPrizeScreen=self))

        print('bruh: %s' % self.panelList)
    ...
    def reload(self):
        ...
        #itemList.sort(lambda a, b: priceSort(a, b, PrizeItem.prizeTypeWeekly))
        #itemList.reverse()

        itemList = PrizeItemList.PrizeItemList().getAllItems()

        for item in itemList:
            self.panelList.append(PrizeItemPanel.PrizeItemPanel(parent=hidden, item=item, type=PrizeItem.PrizeTypePies))
    ...
    def hangUp(self):
        ...
        if not self.prizes:
            ...
            taskMgr.doMethodLater(0, postGoodbyeHangUp, 'postGoodbyeHangUp')
    ...
    def __loadFriend(self):
        if self.allowGetDetails == 0:
            PrizeScreen.notify.warning('smashing requests')
    ...
    def __handleAvatarDetails(self, gotData, avatar, dclass):
        if self.giftAvatar.doId != avatar.doId or gotData == 0:
            PrizeScreen.notify.error('Get Gift Avatar Failed')
            self.gotAvatar = 0
            return
        else:
            self.gotAvatar = 1

...
from toontown.toonfest.DistributedToonfestActivityAI import DistributedToonfestActivityAI
from toontown.toonbase import TTLocalizer
from toontown.parties import PartyGlobals # Still relying on PartyGlobals, soon to be changed

class DistributedToonfestTrampolineActivityAI(DistributedToonfestActivityAI, FSM):
    notify = DirectNotifyGlobal.directNotify.newCategory("DistributedToonfestTrampolineActivityAI")
    
    def __init__(self, air):
        DistributedToonfestActivityAI.__init__(self, air)
    ...
    def awardBeans(self, numBeans, height):
        ...
        self.sendUpdateToAvatarId(avId, 'showJellybeanReward', [reward, av.getTokens(), message])
        av.addTokens(reward)
    ...
    def enterActive(self):
        self.jellybeans = list(range(PartyGlobals.TrampolineNumTokens))

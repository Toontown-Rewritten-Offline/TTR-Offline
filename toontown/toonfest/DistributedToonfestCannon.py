...
from toontown.toonfest.DistributedToonfestCannonActivity import DistributedToonfestCannonActivity
LAND_TIME = 2
WORLD_SCALE = 2.0
...
class DistributedToonfestCannon(DistributedObject, Cannon):
    notify = directNotify.newCategory('DistributedToonfestCannon')
    ...
    def enterCannon(self, avId):
        if avId == base.localAvatar.doId:
            ...
            if not self.activity.hasPlayedBefore():
                ...
                self.acceptOnce(DistributedToonfestCannonActivity.RULES_DONE_EVENT, self.__enableCannonControl)
            else:
    ...
    def exitCannon(self, avId):
        if avId == base.localAvatar.doId:
            ...
            self.ignore(DistributedToonfestCannonActivity.RULES_DONE_EVENT)

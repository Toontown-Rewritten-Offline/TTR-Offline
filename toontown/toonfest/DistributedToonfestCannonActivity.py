...
class DistributedToonfestCannonActivity(DistributedToonfestActivity):
    # I never go anywhere without my party cannon!
    notify = DirectNotifyGlobal.directNotify.newCategory('DistributedToonfestCannonActivity')
    HIT_GROUND = 0
    HIT_TOWER = 1
    HIT_WATER = 2
    REACTIVATE_CLOUD_TASK = 'PartyActivity_ReactivateLastCloud'
    RULES_DONE_EVENT = 'DistributedToonfestCannonActivity_RULES_DONE_EVENT'
    LOCAL_TOON_LANDED_EVENT = 'DistributedToonfestCannonActivity_LOCAL_TOON_LANDED_EVENT'
    NetDivisor = 100
    TimeFactor = 0.75
    BroadcastPeriod = 0.2

    def __init__(self, cr):
        DistributedToonfestActivity.__init__(self, cr, ActivityIds.PartyCannon, ActivityTypes.Continuous, wantRewardGui=True)
        self.gui = None
    ...
    def generateInit(self):
        DistributedToonfestActivity.generateInit(self)
    ...
    def generate(self):
        DistributedToonfestActivity.generate(self)
    ...
    def disable(self):
        ...
        taskMgr.remove(DistributedToonfestCannonActivity.REACTIVATE_CLOUD_TASK)
        self.ignoreAll()
        ...
        if self.hitTrack != None:
            ...
            del self.hitTrack
            self.hitTrack = None
        DistributedToonfestActivity.disable(self)
        return

    def delete(self):
        self.offstage()
        DistributedToonfestActivity.delete(self)
    ...
    def handleRulesDone(self):
        ...
        messenger.send(DistributedToonfestCannonActivity.RULES_DONE_EVENT)
    ...
    def handleRewardDone(self):
        DistributedToonfestActivity.handleRewardDone(self)
    ...
    def __localToonDoneLanding(self):
        ...
        messenger.send(DistributedToonfestCannonActivity.LOCAL_TOON_LANDED_EVENT)
    ...
    def __hitCloudPlatform(self, avatar, collisionEntry, extraArgs = []):
        if not self.hitBumper and not self.hitCloud:
            ...
            if self._lastCloudHit == None:
                ...
                taskMgr.doMethodLater(0.25, self.__reactivateLastCloudHit, DistributedToonfestCannonActivity.REACTIVATE_CLOUD_TASK)
        return
    ...
    def handleToonExited(self, toonId):
        self.notify.debug('DistributedToonfestCannonActivity handleToonExited( toonId=%s ) ' % toonId)

...
from toontown.toonfest.DistributedToonfestActivity import DistributedToonfestActivity
from toontown.parties.activityFSMs import TrampolineActivityFSM
from toontown.parties import PartyUtils

class DistributedToonfestTrampolineActivity(DistributedToonfestActivity):
    ...
    def __init__(self, cr, doJellyBeans = True, doTricks = False, texture = None):
        ...
        DistributedToonfestActivity.__init__(self, cr, PartyGlobals.ActivityIds.PartyTrampoline, PartyGlobals.ActivityTypes.GuestInitiated, wantLever=False, wantRewardGui=True)
        ...
        self.numJellyBeans = PartyGlobals.TrampolineNumTokens
    ...
    def load(self):
        ...
        DistributedToonfestActivity.load(self)
        self.loadModels()
        self.loadCollision()
        self.loadGUI()
        self.loadSounds()
    ...
    def loadModels(self):
        ...
        self.beans = [ loader.loadModelCopy('phase_6/models/events/ttr_m_tf_token') for i in range(self.numJellyBeans) ]
    ...
    def joinRequestDenied(self, reason):
        DistributedToonfestActivity.joinRequestDenied(self, reason)
        self.showMessage(TTLocalizer.PartyActivityDefaultJoinDeny)
        base.cr.playGame.getPlace().fsm.request('walk')

    def exitRequestDenied(self, reason):
        DistributedToonfestActivity.exitRequestDenied(self, reason)
        self.showMessage(TTLocalizer.PartyActivityDefaultExitDeny)

    def setState(self, newState, timestamp):
        DistributedToonfestTrampolineActivity.notify.debug('setState( newState=%s, ... )' % newState)
        DistributedToonfestActivity.setState(self, newState, timestamp)
    ...
    def postHopOn(self):
        ...
        DistributedToonfestActivity.startRules(self)
    ...
    def onJump(self):
        ...
        if self.toon != None and self.toon.getZ() < self.toonPos:
    ...
    def handleToonDisabled(self, toonId):
        DistributedToonfestTrampolineActivity.notify.debug('handleToonDisabled')
        DistributedToonfestTrampolineActivity.notify.debug('avatar ' + str(toonId) + ' disabled')
    ...
    def getTitle(self):
        if self.doJellyBeans:
            ...
            return DistributedToonfestActivity.getTitle(self)
    ...
    def updateTask(self, task):
        ...
        if z <= self.trampHeight:
            ...
            self.surface.setZ(self.trampHeight)
        self.toonIndicator.setZ((z - self.toonPos) * self.indicatorFactor)
        if self.turnLeft:
        ...
        #    self.toon.dropShadow.setAlphaScale(1.0)
        #    self.toon.dropShadow.setScale(1.0)
        if self.leavingTrampoline and z < self.toonPos and abs(a) < 0.1:
            self.releaseToon()
        return Task.cont
    ...
    def simulateStep(self, z):
        if z >= self.toonPos:
            ...
            a = self.g + self.trampK * (self.toonPos - z) - self.trampB * self.toonVelocity
            if self.toonJumped:
        ...
        if topOfJump:
            if newZ > self.toonPos + 20.0:
                self.b_requestAnim('Falling')
            elif self.animFSM.state == 'Jump':
                self.b_requestAnim('Falling')
        if newZ <= self.toonPos and z > self.toonPos:
    ...
    def remoteUpdateTask(self, task):
        if self.toon != None and not self.toon.isEmpty():
            ...
            if z <= self.toonPos:
                ...
                self.surface.setZ(self.toonPos)
        return Task.cont
    ...
    def poofBean(self, bean, beanAnim):
        ...
        poofAnim = Sequence(Parallel(LerpFunc(bean.setAlphaScale, fromData=currentAlpha, toData=0.0, duration=0.25), LerpFunc(bean.setScale, fromData=currentScale, toData=currentScale * 2.0, duration=0.25)), Func(bean.stash), Func(beanAnim.finish), Func(bean.setAlphaScale, currentAlpha), Func(bean.setScale, currentScale))
        poofAnim.start()
        return
